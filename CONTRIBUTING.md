...
- `Dockerfile` describes the pandoc container for building the paper (called by the [Makefile](Makefile))  
...
- `Makefile` contains instructions to build the paper (as pdf or docx), and run the analyses (if a [Makefile](analysis/Makefile) exists in the analysis directory)
...
Note: to avoid handling redundant and possibly outdated files across paper projects, it is recommended to use shared bibliographies, shared templates, and the styles provided by the CSL community.
