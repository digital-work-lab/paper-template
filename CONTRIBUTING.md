...
- `analysis` contains all scripts and corresponding Dockerfile(s) for the analyses
- `data` contains all data (in `raw`, `interim`, and `processed` subdirectories when a more complex data processing pipeline is implemented)
- `figures` contains all conceptual figures
- `output` contains all output generated by the scripts (data plots, tables)
- `Dockerfile` describes the pandoc container for building the manuscript (called by the [Makefile](Makefile))  
- `paper.md` contains the paper
- `readme.md` contains summaries of the paper's revisions (completed and ongoing). Revision sheets are created from this document.
- `Makefile` contains instructions to build the manuscript (as pdf or docx), and run the analyses (if a [Makefile](analysis/Makefile) exists in the analysis directory)
- `.gitignore` contains a list of files that are excluded from git versioning (e.g., output.pdf, which would create conflicts when merging changes)

Shared and symlinked directories (ignored in [.gitignore](git versioning)):

- `bibliography` is a symlink to the shared bibliography
- `styles` is a symlink to a local clone of the [csl styles repository](https://github.com/citation-style-language/styles)
- `templates` is a symlink to the shared template repository

Note: to avoid handling redundant and possibly outdated files across manuscript projects, it is recommended to use shared bibliographies, shared templates, and the styles provided by the CSL community.
Any of the bibliographies (\*.bib), templates (\*.docx, \*.tex), or styles (\*.csl) can also be versioned in the individual manuscript repository.
This can be accomplished by using the code in comments at the beginning of the [Makefile](Makefile).

# Setup

This section explains how to install the environment and build the manuscript.
...
The following steps describe how to
  ...
  - 2. Set up the manuscript production pipeline, and
...
# repo setup:
git init
# MANUALLY create manuscript (update titles etc.)
git add .
...
2) Set up the manuscript production pipeline
...
- Add a line break after each sentence to make the history more readable.
...
```
- Commit trivial changes directly to the `polishing` branch, use dedicated branches otherwise.
- Create a single branch for issues that are related (e.g., group all issues referring to the introduction in the branch 'introduction').
- Create branch iss12 if issue number 12 needs to be discussed (in the pull-request):
```
git checkout main
git checkout -b iss12
```
- While working locally on the iss12, rebase it on main (for better history):
```
git checkout iss12
git rebase main
```
...
git push
```
- When merging shared branches, do not rebase (just merge) when the pull-request history contains discussions/should be preserved (forced-pushes for shared branches erase/modify the github pull-request history).
- Delete merged branches locally and on github (pull-request history is still available on github):
```
git branch -d iss12
```
...
Otherwise: Export manuscript as word file (example filename: 'Manuscript_v11 d091j209fs8.docx').
Version numbers should be incremented with every doc file that is shared externally.
The hash-id should be included as the second part of the filename.
Store doc file locally (to keep track of the version number - hash-id mapping).
Feedback (including tracked changes) should be transferred manually.


## Pre-submission checklist

This checklist can be used to produce camera-ready manuscripts (initials submissions, revisions, and final versions). The checks should be completed in order (keep this in mind when updating this checklist).

### Pre-pre-submission (things that cannot be fixed 1h before the submission deadline)
- [ ] Consider publishing the data/analyses (e.g., on figshare, github, zenodo)
- [ ] Have a knowledgeable colleague read the paper and provide critical and constructive comments
...
- [ ] , which/who
- [ ] i.e., e.g., (with comma)
- [ ] a vs. an.  (use "an" before vowel sounds, e.g., an hour but a horse)
...
### References
 ...
 - [ ] Have all of the references been cited in the text? (in particular if they are cited in figures or tables)
...
- [ ] Check if figures that have been imported from emf-files have a border. The border can be removed with Adobe Acrobat (Edit)
...
- Manuscript repository:
    - [ ] Create YYYY-MM-DD-JOURNAL_Manuscript_ManuscriptID.pdf
    - [ ] Create YYYY-MM-DD-JOURNAL_Submission_proof_ManuscriptID.pdf
    - [ ] Create YYYY-MM-DD-JOURNAL_Revision_sheet_ManuscriptID.pdf
    - [ ] Create YYYY-MM-DD-JOURNAL_Cover_letter_ManuscriptID.pdf
    - [ ] Link in readme.md
    - [ ] Create git-tag
- Analyses repository:
...
- Add the review package to the manuscript directory (YYYY-MM-DD-Outlet-revision-id.pdf)
- Create revision sheet, use [revision-sheet-generator](https://github.com/geritwagner/revision-sheet-generator)
- Check result
- Add table with the revision package to manuscript-repo/readme.md as a separate section at the end
