...
- `analysis` contains all scripts and corresponding Dockerfiles for the analyses
- `bibliography` is a symlink to the shared bibliography
- `data` contains all data (in subdirectories `raw`, `interim`, and `processed` when a more complex data processing pipeline is implemented)
- `figures` contains all conceptual figures
- `output` contains all output generated by the scripts (data plots, tables)
- `styles` is a symlink to the local [csl styles repository](https://github.com/citation-style-language/styles)
- `templates` is a symlink to the shared template repository
- `Dockerfile` describes the pandoc container for building the manuscript (called by the Makefile)  
- `paper.md` contains the paper
- `readme.md` contains summaries of the paper's revisions (completed and ongoing). Revision sheets are generated from this document.
- `Makefile` contains instructions to build the manuscript (as pdf or docx), run the analyses, etc.
...
- Create a copy of the manuscript-template directory
- Execute the following commands in the copy:
...
cd ../..
# create manuscript (update titles etc.)
git add .
...
# MANUALLY update paths in Makefile / install
# To install
make install
...
Tutorials are available [1](https://learngitbranching.js.org/), [2](https://github.com/jlord/git-it-electron).
...
## Retrieving changes from others
...
- Track the deadline
- Create personal revision directory in 3-wip (name: revision-id)
...
- Use (and update) the phrases repository
- Notes:
  - Do not shut down reviewers
  - Prioritize (you cannot address everything)
...
4. Submit
- Create submission documents in readme.md
